...
def search_trending_videos(
    ...
    """
    인기 동영상(트렌드) + 카테고리(videoCategoryId) 검색
    """
    youtube = get_youtube_client()
    take = max(1, min(max_results, 50))
    kwargs = dict(
...
# 검색 버튼 3개 (일반/채널영상/키워드채널)
row1_col1, row1_col2 = st.sidebar.columns(2)
row2_col1, _ = st.sidebar.columns(2)
        ...
        if mode in ("general", "trend", "channel_videos"):
            ...
            for url in thumbs:
                ...
                html_items.append(
                    ...
                    f'    <img src="{url}" class="shorts-img"/>'
                    "  </div>"
                    "</div>"
                )
            html = (
                ...
                ".shorts-img{position:absolute;top:0;left:0;width:100%;height:100%;"
